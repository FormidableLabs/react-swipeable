...
function getHandlers(set, handlerProps) {
  ...
  const onMove = event => {
    set((state, props) => {
      ...
      const velocity = Math.sqrt(absX * absX + absY * absY) / (time || 1); // if swipe is under delta and we have not started to track a swipe: skip update
  ...
  const attachTouch = el => {
    if (el && el.addEventListener) {
      ...
      tls.forEach(([e, h]) => el.addEventListener(e, h)); // return properly scoped cleanup method for removing listeners
...
function useSwipeable(options) {
  ...
  const [handlers, attachTouch] = useMemo(() => getHandlers(cb => transientState.current = cb(transientState.current, transientProps.current), {
    trackMouse
  }), [trackMouse]);
