...
Then, building upon the logic we've built, the Carousel as a functional component is constructed. We hold the number of items within a count of `numItems`. We utilize the reducer within the `React.useReducer` hook.
